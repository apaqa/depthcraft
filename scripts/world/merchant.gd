...
const SHOP_ITEMS := [
	{"id": "bandage", "quantity": 1, "price": 5, "label": "ç¹ƒå¸¶"},
	{"id": "bread", "quantity": 1, "price": 8, "label": "éºµå?"},
	{"id": "seed", "quantity": 3, "price": 3, "label": "ç¨®å? x3"},
	{"id": "iron_ore", "quantity": 5, "price": 15, "label": "?µç¤¦ x5"},
	{"id": "torch", "quantity": 3, "price": 6, "label": "?«æ? x3"},
]
...
func _open_shop() -> void:
	...
	_add_shop_row(vbox, "ç¥žç?è£å?", 50, _on_buy_equipment)
...
func _add_shop_row(parent: Control, label_text: String, price: int, callback: Callable) -> void:
	...
	var lbl := Label.new()
	lbl.text = "%s  ?? %d ?‘å¹£" % [label_text, price]
...
func _update_gold_label() -> void:
	...
	_gold_label.text = "?‘å¹£: %d" % gold_count
