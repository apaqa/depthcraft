...
func _refresh_details() -> void:
	...
	if recipe.get("result_type", "") == "equipment":
		...
		if recipe.has("max_durability"):
			lines.append("?ä?åº? %d/%d" % [int(recipe.get("durability", recipe.get("max_durability", 0))), int(recipe.get("max_durability", 0))])
		if recipe.has("slot"):
			lines.append("æ¬„ä?: %s" % _pretty_name(str(recipe.get("slot", ""))))
	else:
