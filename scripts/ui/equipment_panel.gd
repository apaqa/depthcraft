...
func _build_slot_text(slot_name: String, item: Dictionary) -> String:
	...
	return "%s%s  ?ä?: %d/%d" % [label, player.equipment_system.get_item_display_name(item), durability, max_durability]
...
func _translate_slot(slot_name: String) -> String:
	match slot_name:
		"weapon": return "æ­¦å™¨"
		"helmet": return "?­ç?"
		"chest_armor": return "?¸ç”²"
		"boots": return "?‹å?"
		"accessory": return "é£¾å?"
		"offhand": return "?¯æ?"
		"tool": return "å·¥å…·"
...
func _update_comparison_label() -> void:
	...
	for line in lines:
		var translated = line.replace("ATK", "?»æ?").replace("DEF", "?²ç¦¦").replace("HP", "è¡€??).replace("SPD", "?Ÿåº¦")
