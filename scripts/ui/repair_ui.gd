...
func _refresh() -> void:
	...
	for slot_name in player.equipment_system.get_slot_order():
		...
		var info := Label.new()
		info.text = "?ê‰?: %d/%d" % [durability, max_durability]
		row.add_child(info)
	...
	if not equipped_any:
		detail_label.text = "Ê≤íÊ??ÄË¶Å‰øÆ?ÜÁ?Ë£ùÂ??ÇË??àË??ôÁâ©??
		return
	if not repairable_any:
		detail_label.text = "?Ä?âË??ôËÄê‰?Â∑≤Êªø"
	else:
		detail_label.text = "ÈªûÈÅ∏?ÄË¶Å‰øÆ?ÜÁ??©Â??≤Ë?‰øÆÁ?"
