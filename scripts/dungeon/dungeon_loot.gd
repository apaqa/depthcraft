...
const PREFIXES := ["?Ÿé½??, "?µè£½", "?¼è£½", "?—å½±", "? å¤", "è¢«è??’ç?", "?—ç?ç¦ç?", "è¿…æ·??]
const WEAPON_NAMES := ["?·å?", "?°æ–§", "?å™¨", "?•é?", "?·æ?"]
const HELMET_NAMES := ["?­ç?", "?œå¸½", "?‹å?", "ä¾¿å¸½"]
const CHEST_NAMES := ["?¿ç”²", "?–å???, "?Œå?", "æ³•è?"]
const BOOT_NAMES := ["?´å?", "è­·è…¿", "æ¶¼é?"]
const ACCESSORY_NAMES := ["?’æ?", "è­·èº«ç¬?, "é£¾å?", "?…é?"]
const RARITY_COLORS := {
	"?®é€?: Color(1.0, 1.0, 1.0, 1.0),
	"?ªç?": Color(0.45, 0.95, 0.45, 1.0),
	"ç¨€??: Color(0.42, 0.68, 1.0, 1.0),
	"?²è©©": Color(0.82, 0.45, 1.0, 1.0),
	"?³èªª": Color(1.0, 0.65, 0.1, 1.0),
}
const AFFIX_POOL := [
	{"label": "?»æ?", "stat": "attack", "min": 3, "max": 8},
	{"label": "?²ç¦¦", "stat": "defense", "min": 2, "max": 6},
	{"label": "?€å¤§ç???, "stat": "max_hp", "min": 10, "max": 30},
	{"label": "ç§»é€?, "stat": "speed_multiplier", "min": 5, "max": 15, "scale": 0.01, "suffix": "%"},
	{"label": "?´æ???, "stat": "crit_chance", "min": 3, "max": 8, "scale": 0.01, "suffix": "%"},
	{"label": "?¸è?", "stat": "lifesteal_ratio", "min": 3, "max": 5, "scale": 0.01, "suffix": "%"},
	{"label": "?¡é??Ÿåº¦", "stat": "gather_bonus", "min": 1, "max": 3},
]
const QUALITY_MULTIPLIERS := {
	"?®é€?: 1.0, "?ªç?": 1.3, "ç¨€??: 1.6, "?²è©©": 2.0, "?³èªª": 2.5,
}
const QUALITY_AFFIX_COUNT := {
	"?®é€?: 0, "?ªç?": 1, "ç¨€??: 2, "?²è©©": 3, "?³èªª": 3,
}
...
static func _generate_name(slot: String, quality: String = "?®é€?, rng: RandomNumberGenerator = null) -> String:
