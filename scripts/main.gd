...
func _on_return_to_surface_requested() -> void:
	...
	if hud.has_method("play_transition"):
		await hud.play_transition("è¿”å??°é¢ä¸?..", Color(0, 0, 0, 1), 0.25, 0.15)
	if player != null:
	...
	if player != null:
		player.show_status_message("å·²æŠµ?”ç¬¬ %d å±?| %d ?Šæ®º | %d ?©å??¶é?" % [floor_reached, kill_count, item_count], Color(0.85, 1.0, 0.85, 1.0), 4.0)
...
func _on_player_died() -> void:
	if current_level_id == "dungeon":
		...
		if hud.has_method("play_transition"):
			await hud.play_transition("ä½ æ­»äº†\nç¬?%d å±?| %d ?Šæ®º | ?€?‰åœ°?¢æˆ°?©å?å·²éºå¤±ï?" % [int(current_level.get("current_floor")), int(current_level.get("total_kills"))], Color(0.35, 0.0, 0.0, 1.0), 0.25, 0.15)
		if player != null:
		...
		_broadcast_scene_change("overworld", 1, 0, overworld_return_position if overworld_return_position is Vector2 else Vector2.ZERO, overworld_return_position is Vector2)
		if player != null:
			player.show_status_message("ä½ å¤±?»ä??€?‰æˆ°?©å?ï¼Œè??™å·²?å???, Color(1.0, 0.75, 0.45, 1.0), 3.0)
		if current_level != null and current_level.has_method("trigger_progress_raid") and dungeon_returns_since_raid >= 3:
			current_level.trigger_progress_raid()
...
func _on_floor_transition_requested(next_floor: int) -> void:
	if _is_multiplayer_enabled() and not _is_host():
		...
		return
	if hud.has_method("play_transition"):
		await hud.play_transition("ç¬?%d å±? % next_floor, Color(0, 0, 0, 1), 0.25, 0.05)
