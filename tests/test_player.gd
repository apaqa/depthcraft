...
func test_collision_shape_uses_feet_box() -> void:
	...
	_assert(collision_shape.position == Vector2(0, 2), "Player collision should sit slightly below center for tight wall contact.")
	_assert(rectangle.size == Vector2(4, 3), "Player collision should use the compact feet rectangle.")
